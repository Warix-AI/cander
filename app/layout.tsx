...
const themeScript = `(function(){try{var a=localStorage.getItem('courier-appearance-v2');var theme=null;if(a){try{var j=JSON.parse(a);if(j.colorMode==='dark'||j.colorMode==='dark-charcoal'||j.colorMode==='dark-blue'){theme='dark'}else if(j.colorMode==='light'||j.colorMode==='light-blue'){theme='light'}else if(typeof j.color==='number'){theme=j.color<45?'light':'dark'}}catch(e){}}if(!theme){var t=localStorage.getItem('theme');theme=t==='dark'?'dark':t==='light'?'light':'light'}var root=document.documentElement;if(theme==='dark'){root.classList.add('dark')}else{root.classList.remove('dark')}root.style.colorScheme=theme;var meta=document.querySelector('meta[name="color-scheme"]');if(!meta){meta=document.createElement('meta');meta.setAttribute('name','color-scheme');document.head.appendChild(meta)}meta.setAttribute('content',theme);var color=theme==='dark'?'#0a0a0a':'#ffffff';var tags=document.querySelectorAll('meta[name="theme-color"]');if(tags.length){tags.forEach(function(tag){tag.removeAttribute('media');tag.setAttribute('content',color)})}else{var tc=document.createElement('meta');tc.setAttribute('name','theme-color');tc.setAttribute('content',color);document.head.appendChild(tc)}}catch(e){document.documentElement.classList.remove('dark');document.documentElement.style.colorScheme='light'}})();`;
