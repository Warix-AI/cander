/**
 * Client mirror of web-retrieval — keep in sync with
...
export function hydrateEvidenceFromSession(
  ...
  userRequest: string,
  requestedDomain?: string | null,
): Array<EvidenceItemLite & { publishedAt?: string | null; retrievedAt: string }> {
  ...
  return ranked.map((hit) => ({
    ...
    metadata: { discoveryOnly: true, rankScore: hit.score, reusedSession: session.id },
  }));
}
...
/** Simulated retrieval loop for unit tests (no network). */
export function simulateRetrievalLoop(steps: Array<{
  action: "search" | "open" | "check" | "answer";
  snippets?: number;
  pages?: number;
}>): {
  searches: number;
  ...
  let pageCount = 0;
  let answered = false;
  let blockedOnSnippets = false;

  for (const step of steps) {
    if (step.action === "search") {
      searches++;
      snippetCount += step.snippets ?? 3;
    }
    if (step.action === "open") {
      opens++;
      pageCount += step.pages ?? 1;
    }
    if (step.action === "check") {
      if (snippetCount > 0 && pageCount === 0) blockedOnSnippets = true;
    }
    if (step.action === "answer") {
      if (snippetCount > 0 && pageCount === 0) {
        blockedOnSnippets = true;
        continue;
      }
      answered = true;
      break;
  ...
  return { searches, opens, answered, blockedOnSnippets };
}
