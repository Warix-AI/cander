/**
 * Orchestration policy — client mirror for tests.
 * Keep in sync with supabase/functions/_shared/agent/policy.ts
 */
...
export function detectLiveInformation(content: string): LiveInfoSignal {
  ...
  return {
    ...
    publicInfo: needsWeb,
    reason: needsWeb ? `live:${signals.join("+")}` : "not_live",
    signals,
  };
}
...
export function broadNewsSearchQueries(_userContent: string): string[] {
  return [
...
export function primarySearchQuery(
  userContent: string,
  live: LiveInfoSignal,
): string {
...
export function scrubLimitationBoilerplate(text: string): string {
  ...
  out = out.replace(/\bMy knowledge (cutoff|cut-off)[^.!?]*[.!?]\s*/gi, "");
  out = out.replace(
