...
`STATE → HYDRATE → INTERPRET/NORMALIZE → RUN → VERIFY → ANSWER → COMMIT`
...
- **Knowledge route:** `LOCAL` | `WEB_REQUIRED` | `UNCERTAIN` before tools (no numeric confidence). Auto: LOCAL answers locally; WEB_REQUIRED/UNCERTAIN → Exa Deep. Composer override `browserMode` / `cander:web-mode` / `NEXT_PUBLIC_CANDER_WEB_MODE`: Auto | On | Off. **Default: Off** (no web unless user explicitly asks to browse/search, or mode is set to Auto/On).
- **Web retrieval:** open-web `WEB` intents always use Exa Search **`type: "deep"`**. Preserve coherent multi-fact asks (e.g. combined calories across brands) as **one** Exa Deep query — only split when independent, different capabilities, or real deps (WEB→CALENDAR). Validated Exa text returns as-is. `[WEB_TURN_TRACE]` logs raw text, route, splitDecision, Exa queries/responses, accept/reject, final answer, lossHint.
