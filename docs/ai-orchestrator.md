...
- **Web retrieval:** open-web `WEB` intents always use Exa Search **`type: "deep"`** only (no instant/fast/auto/deep-lite/deep-reasoning/agent selection). Apple FM decides whether WEB is needed and emits a canonical query; Exa Deep answers; Candor validates and returns Exa text essentially as-is (no FM rewrite of web facts). Explicit URL/domain still **direct-fetch** first. Independent intents run concurrent Exa Deep calls; one refined-query retry on validation failure; then unresolved. Citations: strongest 1–3.
